...
- У каждого напоминания есть inline-кнопки:
  - ⏸ Отложить (пресеты: 10 мин, 30 мин, 2 часа, 1 день).
  - ✏ Перенести — бот попросит новую дату/время и подтвердит действие.
  - 🗑 Удалить — отключает напоминание.
- Поддерживаемые повторы: ежедневно, по будням, еженедельно (по выбранным дням), ежемесячно (по числу месяца).
