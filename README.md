...
- `status: "ok" | "refused" | "error" | "ratelimited"` — статус результата.
...
## Telegram smoke-test (Stage 1)
1. `/menu` → появляются inline-кнопки.
2. Нажать “🧮 Calc” → бот отвечает “Calc: /calc <выражение>.”.
3. `/reminders 5` → список + кнопки “🔕 Off”.
4. Нажать “🔕 Off” → “Напоминание отключено …” (или refused, если уже выключено).
...
- `/reminders` — ближайшие напоминания (с кнопками управления).
- `/tasks` — список задач.
...
## Напоминания
- `/reminders [N]` — список ближайших напоминаний (по умолчанию 5).
- У каждого напоминания есть inline-кнопки:
  - ⏸ Отложить (пресеты: 10 мин, +30 мин, +2 часа).
  - ✏ Перенести — бот попросит новую дату/время и подтвердит действие.
  - 🗑 Отключить — отключает напоминание.
- Создание из меню: ⏰ Напоминания → ➕ Создать (wizard без LLM: текст → дата/время (МСК) → повтор → подтверждение).
- Поддерживаемые повторы: daily, weekdays, weekly (по выбранным дням 0..6), monthly (по числу месяца).
