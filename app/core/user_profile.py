...
class ReminderDefaults:
    ...
    def from_dict(payload: dict[str, Any] | None) -> ReminderDefaults:
        ...
        return ReminderDefaults(
            ...
            offset_minutes=_coerce_int(offset_minutes, DEFAULT_REMINDER_OFFSET_MINUTES, min_value=0),
        )
...
class UserProfile:
    ...
    @staticmethod
    def from_dict(payload: dict[str, Any] | None) -> UserProfile:
        if not isinstance(payload, dict):
...
def normalize_profile_payload(payload: dict[str, Any] | None) -> dict[str, Any]:
