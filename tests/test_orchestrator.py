...
def test_orchestrator_disabled_task(tmp_path: Path) -> None:
    ...
    execution = orchestrator.execute_task(user_id=1, task_name="upper", payload="hello")

    assert execution.status == "error"
    assert "disabled" in execution.result
...
def test_orchestrator_records_last_execution(tmp_path: Path) -> None:
    ...
    orchestrator.execute_task(user_id=42, task_name="upper", payload="hello")
